...
// class DemosActivity : VMActivity<DemosViewModel, ActivityDemosBinding>() {
    ...
    private val mUtilsFragment by lazy {
        ARouter.getInstance().build(MyARoute.Utils_UtilsFragment).navigation() as Fragment
    }

    private val mViewsFragment by lazy {
        ARouter.getInstance().build(MyARoute.View_ViewsFragment).navigation() as Fragment
    }

    private val mFunctionFragment by lazy {
        ARouter.getInstance().build(MyARoute.Function_FunctionFragment)
