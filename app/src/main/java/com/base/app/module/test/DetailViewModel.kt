...
class DetailViewModel @ViewModelInject constructor(private val mRepository: DetailRepository) :
     ...
     * 获取首页文章列表
     */
    private val articleLiveData by lazy { MutableLiveData<BResponse<WanArticle>>() }
    fun getArticleLiveData(): LiveData<BResponse<WanArticle>> = articleLiveData

    fun getArticle() {
        rxLifeScope.launch {
            mRepository.getArticle()
                .onStart { sendState(ResponseState.Loading(BConstant.article)) }
                .onCompletion { sendState(ResponseState.Completed(BConstant.article)) }
                .collectLatest {
                    if (it.isSuccess()) articleLiveData.value = it
                }
        }
    }
