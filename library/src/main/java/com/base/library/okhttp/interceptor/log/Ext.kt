...
internal fun String.appendLine(): String {
    if (length >= LogInterceptor.MAX_LINE_LENGTH) {
        return this
    }
    val sb = StringBuilder(this)
    repeat(LogInterceptor.MAX_LINE_LENGTH - length) {
        sb.append(LogInterceptor.CLINE)
    }
    return sb.toString()
}
...
fun RequestBody.formatAsPossible(
    ...
    maxLineLength: Int = LogInterceptor.MAX_LINE_LENGTH
): List<String> {
...
private fun String.separateByLength(maxLineLength: Int = LogInterceptor.MAX_LINE_LENGTH): List<String> {
    if (isNullOrEmpty()) {
        return emptyList()
    }
